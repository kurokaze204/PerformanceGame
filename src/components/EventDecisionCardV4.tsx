...
const expertTravelCost=(from:string,target?:string)=>{
 if(!target||from===target)return 0;
 const a=cityPoint(from);const b=cityPoint(target);if(!a||!b)return 0;
 const distance=Math.hypot(a.x-b.x,a.y-b.y);
 const scaled=Math.max(0,Math.min(1,(distance-minCityDistance)/(maxCityDistance-minCityDistance)));
...
export const EventDecisionCardV4:React.FC<Props>=({session,company,event,cardNumber,onSetAllocation,onResolveEvent,onAcknowledgeResolution,onRedrawEvent,canHorizonRedraw=false})=>{
 ...
 const committedTravel=useMemo(()=>{const seen=new Set<string>();let total=0;for(const item of allocations){if(!item.expertId||seen.has(item.expertId))continue;seen.add(item.expertId);total+=item.expertTravelCost||0;}return total},[allocations]);
 const committedConsultant=event.consultantSpend||allocations.reduce((s,x)=>s+(x.consultantCost||0),0);
 const interventionCost=committedTravel+(event.isResolved?committedConsultant:consultantCost);
 ...
 const networkAvailable=activeDomain?copMembershipActiveV4(session,company.id,activeDomain):false;
 const networkPeerScore=activeDomain?copPeerKnowledgeScoreV4(session,company,activeDomain):0;
 ...
 return <motion.div ref={containerRef} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="relative w-full grid lg:grid-cols-[minmax(0,1fr)_300px] gap-3 items-start">
  ...
  <div className="relative z-[2] space-y-2">
   ...
   <section ref={detailRef} className="relative z-[3] rounded-xl border-2 border-violet-400 bg-violet-950/90 p-3 min-h-[100px] shadow-xl shadow-violet-950/40">
    ...
    {mode==='expert'&&<><div className="font-black text-sm text-violet-100">Ask one of our experts — {DOMAIN_INFO[activeDomain].label}</div><div className="grid grid-cols-2 gap-2 mt-2">{eligibleExperts.map(e=>{const chosen=allocation.expertId===e.id;const travel=chosen?0:expertTravelCost(e.location,event.targetSiteId);return <button key={e.id} onClick={()=>setCurrent({expertId:chosen?'':e.id,expertTravelCost:travel})} className={`rounded-lg border p-2 text-left ${chosen?'bg-amber-400 text-slate-950 border-amber-200':'bg-slate-950/80 border-violet-700'}`}><b className="text-xs">{e.name}</b><div className="text-[9px]">{DOMAIN_INFO[activeDomain].label} {e.domains.find(x=>x.domain===activeDomain)?.score}</div>{travel>0&&!chosen&&<div className="text-[9px] text-amber-300 mt-1">Travel {formatCurrency(travel)}</div>}</button>})}</div></>}
    {mode==='network'&&<><div className="font-black text-sm text-violet-100">Ask our network — {DOMAIN_INFO[activeDomain].label}</div>{networkAvailable?<><div className="text-[10px] text-violet-200/70 mt-1">Active CoP · peer knowledge score {networkPeerScore} · +{session.config.cop_support_bonus} challenge support</div><button onClick={()=>setCurrent({useCoPSupport:!allocation.useCoPSupport})} className="mt-2 rounded-lg bg-amber-400 text-slate-950 px-3 py-2 text-xs font-black">{allocation.useCoPSupport?'NETWORK SELECTED · REMOVE':`USE COMMUNITY OF PRACTICE (+${session.config.cop_support_bonus})`}</button></>:<div className="text-xs text-violet-200/70 mt-2">No active Community of Practice in this domain. Join one during Invest to unlock network support for the next two rounds.</div>}</>}
    {mode==='reputation'&&<div className="flex justify-between items-center gap-4"><div><b className="text-sm text-violet-100">Call in a favour</b><div className="text-3xl leading-none font-black text-white mt-1">{favoursRemaining}/{FAVOUR_LIMIT}</div><div className="text-xs text-violet-200/70 mt-1">Favours remaining. Guarantees this challenge without boosting knowledge capability.</div></div><button onClick={useReputation} disabled={favoursRemaining<=0||busy} className="rounded-lg bg-amber-400 text-slate-950 px-3 py-2 text-xs font-black disabled:opacity-40 disabled:cursor-not-allowed">{favoursRemaining<=0?'NO FAVOURS LEFT':busy?'USING…':'USE 1 FAVOUR'}</button></div>}
    {mode==='consultant'&&<><div className="font-black text-sm text-violet-100">External expertise — {DOMAIN_INFO[activeDomain].label} <span className="text-amber-400 text-[10px]">NON-PREFERRED</span></div><div className="flex gap-2 mt-2">{[1,2,3].map(p=><button key={p} onClick={()=>setCurrent({consultantPoints:allocation.consultantPoints===p?0:p})} className={`rounded-lg border px-3 py-2 font-black text-xs ${allocation.consultantPoints===p?'bg-amber-400 text-slate-950 border-amber-200':'bg-slate-950/80 border-violet-700'}`}>+{p}<span className="block text-[9px]">from ${consultantRate*p}k</span></button>)}</div><div className="text-[10px] text-violet-200/60 mt-2">Choose +1, +2 or +3 knowledge points. Extra points can take the probability to 100%; you still pay for the level selected.</div></>}
  ...
  <aside className="relative z-[2] space-y-1.5"><div className="text-[9px] uppercase tracking-wider text-slate-400 font-black">Knowledge Suite</div>{MODES.map(m=>{const Icon=m.icon;const reputationEmpty=m.id==='reputation'&&favoursRemaining<=0;return <button ref={node=>{modeRefs.current[m.id]=node}} key={m.id} disabled={reputationEmpty} onClick={()=>chooseMode(m.id)} className={`relative w-full rounded-lg border px-3 py-2 text-left ${mode===m.id?'border-violet-300 bg-violet-950/90 shadow-lg shadow-violet-950/40':'border-slate-700 bg-slate-900/95'} ${m.id==='consultant'?'opacity-80':''} ${reputationEmpty?'opacity-35 cursor-not-allowed':''}`}><div className="flex gap-2 items-center"><Icon className="w-4 h-4 text-indigo-300"/><div><b className="text-xs text-white">{m.label}</b>{m.id==='reputation'?<div className="text-xl leading-none font-black text-white mt-1">{favoursRemaining}/{FAVOUR_LIMIT}</div>:<div className="text-[9px] text-slate-400">{m.sub}</div>}</div></div>{selected[m.id]&&<Check className="absolute right-2 top-2 w-4 h-4 text-emerald-300"/>}</button>})}<button onClick={resolve} disabled={busy} className="w-full rounded-xl bg-indigo-500 text-white py-3 text-lg font-black disabled:opacity-50">GO</button></aside>
 </motion.div>;
};
