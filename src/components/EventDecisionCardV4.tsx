...
export const EventDecisionCardV4:React.FC<Props>=({session,company,event,cardNumber,onSetAllocation,onResolveEvent,onAcknowledgeResolution,onRedrawEvent,canHorizonRedraw=false})=>{
 ...
 const continueResolution=async()=>{
  ...
  setBusy(true);
  try{
   if(resolution.reputation){
    const freshRes=await fetch(`/api/sessions/${session.id}`);
    if(freshRes.ok){
     const freshSession=await freshRes.json();
     await onAcknowledgeResolution({session:freshSession});
     return;
    }
   }
