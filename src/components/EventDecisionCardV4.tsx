 ...
 return <motion.div ref={containerRef} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="relative w-full grid lg:grid-cols-[minmax(0,1fr)_300px] gap-3 items-start">
  ...
  <div className="relative z-[2] space-y-2">
   ...
   <section ref={detailRef} className="relative z-[3] rounded-xl border-2 border-violet-400 bg-violet-950/90 p-3 min-h-[100px] shadow-xl shadow-violet-950/40">
    ...
    {mode==='reputation'&&<div className="flex justify-between items-center gap-4"><div><b className="text-sm text-violet-100">Call in a favour</b><div className="text-3xl leading-none font-black text-white mt-1">{favoursRemaining}/{FAVOUR_LIMIT}</div><div className="text-xs text-violet-200/70 mt-1">Favours remaining. Guarantees this challenge without creating knowledge capability.</div></div><button onClick={useReputation} disabled={favoursRemaining<=0||busy} className="rounded-lg bg-amber-400 text-slate-950 px-3 py-2 text-xs font-black disabled:opacity-40 disabled:cursor-not-allowed">{favoursRemaining<=0?'NO FAVOURS LEFT':busy?'USING…':'USE 1 FAVOUR'}</button></div>}
