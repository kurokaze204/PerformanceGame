 ...
 return <motion.div ref={containerRef} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="relative w-full grid lg:grid-cols-[minmax(0,1fr)_300px] gap-3 items-start">
  ...
  <div className="relative z-[2] space-y-2">
   ...
   <section ref={detailRef} className="relative z-[3] rounded-xl border-2 border-violet-400 bg-violet-950/90 p-3 min-h-[100px] shadow-xl shadow-violet-950/40">
    ...
    {mode==='reputation'&&<div className="flex justify-between items-center"><div><b className="text-sm text-violet-100">Call in a favour</b><div className="text-xs text-violet-200/70">{company.reputationPoints} reputation left. Guarantees this challenge without creating knowledge capability.</div></div><button onClick={useReputation} disabled={company.reputationPoints<=0||busy} className="rounded-lg bg-amber-400 text-slate-950 px-3 py-2 text-xs font-black disabled:opacity-50">{busy?'USING…':'USE 1'}</button></div>}
  ...
  <aside className="relative z-[2] space-y-1.5"><div className="text-[9px] uppercase tracking-wider text-slate-400 font-black">Knowledge Suite</div>{MODES.map(m=>{const Icon=m.icon;return <button ref={node=>{modeRefs.current[m.id]=node}} key={m.id} onClick={()=>chooseMode(m.id)} className={`relative w-full rounded-lg border px-3 py-2 text-left ${mode===m.id?'border-violet-300 bg-violet-950/90 shadow-lg shadow-violet-950/40':'border-slate-700 bg-slate-900/95'} ${m.id==='consultant'?'opacity-80':''}`}><div className="flex gap-2 items-center"><Icon className="w-4 h-4 text-indigo-300"/><div><b className="text-xs text-white">{m.label}</b><div className="text-[9px] text-slate-400">{m.sub}</div></div></div>{selected[m.id]&&<Check className="absolute right-2 top-2 w-4 h-4 text-emerald-300"/>}</button>})}<button onClick={resolve} disabled={busy} className="w-full rounded-xl bg-indigo-500 text-white py-3 text-lg font-black disabled:opacity-50">GO</button></aside>
 </motion.div>;
};
