...
export const SharedGameTimerV2: React.FC<Props> = ({ session, onResetGame, resetting=false, onFacilitatorLogin, onBreakoutRooms }) => {
  ...
  return <div className="flex items-center justify-end gap-2">
    {teamStatus && <div aria-live="polite" className="tpg-status-tile"><span className={`h-2.5 w-2.5 rounded-full shrink-0 ${teamStatus==='Complete tasks below'?'bg-indigo-400':teamStatus.startsWith('Waiting')?'bg-amber-400':'bg-emerald-400'}`}/><span>{teamStatus}</span></div>}
    <div ref={wrapperRef} className="relative">
      <button type="button" onClick={()=>setOpen(v=>!v)} className={`tpg-top-action min-w-[150px] text-white ${tone}`} aria-expanded={open} aria-haspopup="menu" aria-label={clockLabel}>
      ...
      {open&&<div role="menu" className="absolute right-0 top-full mt-2 z-[120] w-56 rounded-2xl border border-slate-700 bg-slate-950/98 p-2 shadow-2xl">
        <div className="flex items-center justify-between px-2 py-1"><div className="text-[10px] uppercase tracking-wider text-slate-500 font-black">Game controls</div><button onClick={()=>setOpen(false)} className="tpg-close-button !h-8 !w-8" aria-label="Close game controls"><X className="w-4 h-4"/></button></div>
        {facilitatorGameView?<button role="menuitem" onClick={reopenControlRoom} className="tpg-action mt-1 w-full !h-10 !text-xs"><Settings2 className="w-4 h-4"/>Control room</button>:onFacilitatorLogin&&<button role="menuitem" onClick={()=>{setOpen(false);onFacilitatorLogin()}} className="mt-1 w-full rounded-xl border border-violet-800 bg-violet-950/35 px-3 py-2.5 text-xs font-black text-violet-200 flex items-center justify-center gap-2"><LogIn className="w-4 h-4"/>Facilitator login</button>}
        {onBreakoutRooms&&<button role="menuitem" onClick={()=>{setOpen(false);onBreakoutRooms()}} className="tpg-action tpg-action-muted mt-1 w-full !h-10 !text-xs"><Users className="w-4 h-4"/>Breakout rooms</button>}
        {isSolo&&!roundMode&&<button role="menuitem" disabled={timerBusy} onClick={()=>void timerCommand(isRunning?'pause':'start')} className="tpg-action mt-1 w-full !h-10 !text-xs border-emerald-700 bg-emerald-950/35 text-emerald-200 hover:border-emerald-400 disabled:opacity-50">{isRunning?<Pause className="w-4 h-4"/>:<Play className="w-4 h-4"/>}{isRunning?'Pause clock':'Play clock'}</button>}
        {onResetGame&&<button role="menuitem" disabled={resetting} onClick={()=>void onResetGame()} className="tpg-action mt-2 w-full !h-10 !text-xs border-amber-700 bg-amber-950/30 text-amber-200 hover:border-amber-400 disabled:opacity-50"><RotateCcw className="w-4 h-4"/>{resetting?'Resetting…':'Reset game'}</button>}
      </div>}
    </div>
  </div>;
};
