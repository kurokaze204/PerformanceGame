...
export const BoardToolTabsV1:React.FC<Props>=({session,company,selectedSiteId,tool,onTool,onSelectSite,onSelectHQ,onSelectExpert})=>{const domains=domainsForMode(session.experienceMode),site=company.sites.find(s=>s.id===selectedSiteId)||company.sites[0];return <aside className="relative z-40 flex shrink-0 self-stretch min-h-[560px] sm:min-h-[620px]">
 <nav className="flex w-[88px] sm:w-[108px] flex-col gap-2 pt-12" aria-label="Board tools">{tabs.map(([id,label,Icon])=><button key={id} onClick={()=>onTool(tool===id?null:id)} className={`min-h-14 rounded-l-2xl border-2 border-r-0 px-2 py-3 flex flex-col sm:flex-row items-center justify-center gap-1.5 text-sm sm:text-base font-black transition ${tool===id?'bg-violet-950 border-violet-300 text-white':'bg-[#10151f] border-slate-700 text-slate-200 hover:border-emerald-400'}`}><Icon className="w-5 h-5 shrink-0"/><span>{label}</span></button>)}</nav>
 {tool&&<div className="w-[min(430px,42vw)] min-w-[320px] bg-[#0b0f18] border-l-2 border-violet-500 overflow-y-auto p-5"><div className="flex justify-between items-center mb-4"><div className="text-sm uppercase tracking-[.18em] text-emerald-300 font-black">Board tool</div><button onClick={()=>onTool(null)} className="w-12 h-12 grid place-items-center rounded-xl border border-slate-700"><X className="w-6 h-6"/></button></div>
...
const Stat:React.FC<{label:string;value:string}>=({label,value})=><div className="rounded-2xl border-2 border-slate-800 bg-slate-950 p-4"><div className="text-xs uppercase text-slate-500 font-black">{label}</div><div className="text-2xl font-black text-emerald-300">{value}</div></div>;
