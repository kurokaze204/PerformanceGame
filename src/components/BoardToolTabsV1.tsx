...
export const BoardToolTabsV1:React.FC<Props>=({session,company,selectedSiteId,tool,onTool,onSelectSite,onSelectHQ,onSelectExpert,onOpenCharts,onSessionUpdate})=>{
 ...
 if(session.phase==='investment')return <InvestmentDecisionDockV1 session={session} company={company} selectedSiteId={selectedSiteId} tool={tool} onTool={onTool} onSelectSite={onSelectSite} onSelectHQ={onSelectHQ} onSelectExpert={onSelectExpert} onOpenCharts={onOpenCharts} onSessionUpdate={onSessionUpdate}/>;
