   ...
   <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
    <section className="p-5 sm:p-7">
     ...
     {mode==='create'&&<div className="mt-6 space-y-5">
      ...
      <div className="grid sm:grid-cols-2 gap-3">{experienceMode==='expert'&&gameEndMode==='rounds'?<Field label="Rounds before Final Challenge"><input type="number" min={1} max={200} value={finalRoundCount} onChange={e=>setFinalRoundCount(Math.max(1,Math.min(200,Number(e.target.value))))} className="control"/></Field>:<Field label="Game length"><select value={duration} onChange={e=>setDuration(Number(e.target.value))} className="control"><option value={30}>30 minutes</option><option value={45}>45 minutes</option><option value={60}>60 minutes</option><option value={90}>90 minutes</option></select></Field>}<Field label="Actions per company per round"><input type="number" min={1} max={10} value={actionsPerRound} onChange={e=>setActionsPerRound(Math.max(1,Math.min(10,Number(e.target.value))))} className="control"/></Field></div>
