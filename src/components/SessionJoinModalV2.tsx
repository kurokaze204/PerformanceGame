...
export const SessionJoinModalV2:React.FC<Props>=({currentSession,onJoinSession,initialMode='join',onClose})=>{
 ...
 return <div className="fixed inset-0 z-[250] grid place-items-center bg-[#080b12]/95 p-4" role="dialog" aria-modal="true" aria-labelledby="join-title">
  <div className="relative w-full max-w-xl rounded-3xl border border-slate-700 bg-slate-900 p-6 text-slate-200 shadow-2xl">
   ...
   <div className="mt-5 grid grid-cols-3 gap-1 rounded-xl border border-slate-700 bg-slate-950 p-1" role="tablist" aria-label="Game entry options">
   ...
   {mode==='join'&&<div className="mt-5 space-y-3">
    ...
    <button disabled={!hasName||!sessionId.trim()} onClick={()=>{rememberName();onJoinSession(sessionId.trim(),' ',playerName.trim())}} className="primary disabled:cursor-not-allowed disabled:opacity-40">CONNECT TO GAME <ArrowRight className="h-4 w-4"/></button>
   ...
   {mode==='create'&&<div className="mt-5 space-y-4">
    ...
    <div className="rounded-xl border border-indigo-800 bg-indigo-950/25 p-3 text-xs text-slate-300">Defaults are tuned by mode: Newbie starts at 5 Actions per company per round; Expert starts at 3 to create stronger portfolio trade-offs.</div>
    ...
    <button disabled={!hasName||creating} onClick={()=>void createAndJoin(companyCount,false)} className="primary disabled:cursor-not-allowed disabled:opacity-40"><Sparkles className="h-4 w-4"/>{creating?'CREATING…':'LAUNCH NEW GAME'}</button>
    <button disabled={!hasName||creating} onClick={()=>void createAndJoin(1,true)} className="w-full rounded-xl border border-emerald-800 bg-emerald-950/25 py-2.5 text-xs font-black text-emerald-300 disabled:cursor-not-allowed disabled:opacity-40">LAUNCH 1-COMPANY SOLO GAME · CLOCK STARTS AUTOMATICALLY</button>
   </div>}
  </div>
 </div>;
};
