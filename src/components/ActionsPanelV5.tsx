...
const ANCHORS:{id:AnchorId;title:string;description:string;disabled?:boolean;icon:React.ElementType}[]=[
 ...
 {id:'risk',title:'Anticipate knowledge risk',description:'Boost foresight so risk can be handled more deliberately.',icon:Radar},
];
const INTERVENTIONS:Intervention[]=[
 ...
 {id:'aar',title:'Lessons Learned / AAR',description:'Turn one of this round’s completed challenges into +1 local team or codified capability.',anchor:'existing',icon:Sparkles,actionType:'LESSONS_LEARNED'},
...
function anchorVisible(session:GameSessionV2,id:AnchorId){
 if(session.experienceMode==='expert')return true;
 if(id==='existing'||id==='favour'||id==='external')return true;
 if(id==='expert')return capabilityUnlocked(session.experienceMode,session.round,'expert');
 if(id==='network')return capabilityUnlocked(session.experienceMode,session.round,'network');
 return capabilityUnlocked(session.experienceMode,session.round,'foresight');
}
...
export const ActionsPanelV5:React.FC<Props>=({session,company,onPerformAction,onNextPhase})=>{
 ...
 const relevantDomains=useMemo(()=>{
   ...
   if((selectedId==='transfer'||selectedId==='join-cop')&&selectedExpert){const held=new Set(selectedExpert.domains.map(d=>d.domain));return DOMAINS.filter(d=>held.has(d));}
   return DOMAINS;
 },[selectedId,selectedAarEvent,selectedExpert]);
 ...
 const expected=useMemo(()=>{
   ...
   if(selectedId==='aar'){const b=learningTarget==='team'?(selectedSite?.teamCapability[domain]??0):(selectedSite?.codifiedKnowledge[domain]??0);return selectedAarEvent&&selectedSite?`AAR on “${selectedAarEvent.card.title}” → ${selectedSite.name} ${DOMAIN_INFO[domain].label} ${learningTarget==='team'?'Team':'Docs'} ${b} → ${Math.min(6,b+1)}`:'Choose a completed challenge';}
 ...
 },[selectedId,selectedSite,selectedExpert,selectedExpertSkill,selectedAarEvent,domain,learningTarget,activeSites,company.intranet,company.automatedDomains,session.round,session.config]);
 ...
 <div className="fixed left-3 right-3 top-[94px] bottom-3 z-[90] rounded-3xl border border-slate-700 bg-[#080b12]/[0.985] shadow-2xl p-3 overflow-hidden flex flex-col" role="region" aria-label="Investment actions">
   ...
   <div className="mt-3 grid grid-cols-[230px_minmax(0,1fr)_320px] gap-3 min-h-0 flex-1">
     ...
     <div className="min-h-0"><div className="text-[9px] uppercase tracking-wider text-emerald-400 font-black mb-1.5">Knowledge-building interventions — yellow arrows show which capability each intervention boosts</div><div className="grid grid-cols-3 gap-2">{visibleInterventions.map(item=>{const Icon=item.icon;const active=item.id===selectedId;const cost=INVESTMENT_COSTS_V4[item.actionType]||0;return <button key={item.id} onClick={()=>setSelectedId(item.id)} className={`rounded-xl border-2 px-3 py-2 text-left min-h-[92px] ${active?'border-white bg-emerald-800':'border-emerald-800 bg-emerald-950/75 hover:border-emerald-500'}`}><div className="flex items-start gap-2"><Icon className="w-4 h-4 text-emerald-300 shrink-0 mt-0.5"/><div className="min-w-0"><div className="flex justify-between gap-2"><b className="text-sm text-white">{item.title}</b><span className="text-[10px] font-black text-amber-300 whitespace-nowrap">{formatCurrency(cost)}</span></div><div className="text-[10px] text-emerald-100/70 leading-snug">{item.description}</div></div></div><div className="mt-1.5 flex items-center gap-1 text-amber-300 text-[10px] font-black"><ArrowLeft className="w-4 h-4 stroke-[3]"/> boosts: {anchorTitle(item.anchor)}</div></button>})}</div></div>
     <div className="rounded-2xl border border-slate-600 bg-slate-900 p-3 min-h-0 overflow-auto"><div className="text-[9px] uppercase tracking-wider text-indigo-300 font-black">Investment action</div><h3 className="text-lg font-black text-white">{selected.title}</h3><div className="mt-2 space-y-2">
       {selectedId==='aar'&&<label className="block text-[9px] uppercase text-slate-500 font-black">Recent challenge<select value={selectedAarEvent?.instanceId||''} onChange={e=>setAarEventId(e.target.value)} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white normal-case">{resolvedEvents.map(e=><option key={e.instanceId} value={e.instanceId}>{e.success===false?'FAIL':'SUCCESS'} · {e.card.title}</option>)}</select></label>}
       {needsSite&&<label className="block text-[9px] uppercase text-slate-500 font-black">Site<select value={siteId} disabled={aarSiteLocked} onChange={e=>setSiteId(e.target.value)} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white normal-case disabled:opacity-60">{activeSites.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}
       {needsExpert&&<label className="block text-[9px] uppercase text-slate-500 font-black">Expert<select value={expertId} onChange={e=>setExpertId(e.target.value)} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white normal-case">{activeExperts.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>}
       {needsExpert&&selectedExpert&&<div className="rounded-xl border border-violet-700 bg-violet-950/35 p-2"><div className="flex items-start justify-between gap-2"><div><div className="text-sm font-black text-white">{selectedExpert.name}</div><div className="text-[10px] text-violet-200/70">{expertLocation}</div></div>{selectedExpert.isSPOF&&<span className="rounded-full border border-rose-700 bg-rose-950 px-2 py-0.5 text-[9px] font-black text-rose-200">SPOF</span>}</div><div className="grid grid-cols-2 gap-1 mt-2">{selectedExpert.domains.map(skill=><div key={skill.domain} className={`rounded-lg border px-2 py-1.5 ${skill.domain===domain?'border-amber-300 bg-amber-950/40':'border-violet-900 bg-slate-950/70'}`}><div className="text-[9px] text-slate-400">{DOMAIN_INFO[skill.domain].label}</div><div className="text-lg font-black text-white">{skill.score}</div></div>)}</div><div className="mt-2 text-[9px] text-violet-200/70">Experts can take part in multiple Invest interventions; each intervention still costs 1 Action and its listed $ cost.</div></div>}
       <label className="block text-[9px] uppercase text-slate-500 font-black">Domain<select value={domain} onChange={e=>setDomain(e.target.value as KnowledgeDomain)} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white normal-case">{relevantDomains.map(d=><option key={d} value={d}>{DOMAIN_INFO[d].label}</option>)}</select></label>
       {selectedId==='aar'&&<label className="block text-[9px] uppercase text-slate-500 font-black">Capture as<select value={learningTarget} onChange={e=>setLearningTarget(e.target.value as 'team'|'codified')} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white normal-case"><option value="team">Team Capability</option><option value="codified">Local Codified Knowledge</option></select></label>}
     </div>
