...
export const ActionsPanelV5:React.FC<Props>=({session,company,onPerformAction,onNextPhase})=>{
 ...
 <div className="fixed left-3 right-3 min-[1280px]:right-[360px] top-[94px] bottom-[72px] z-[90] rounded-3xl border border-slate-700 bg-[#080b12]/[0.985] shadow-2xl p-4 overflow-hidden flex flex-col" role="region" aria-label="Investment actions">
   ...
   <div className="mt-3 flex min-h-0 flex-1 flex-col gap-5">
     <div className="flex h-[clamp(300px,43vh,420px)] shrink-0 gap-3">
       ...
       <aside className="relative flex w-[290px] shrink-0 flex-col overflow-visible rounded-2xl border border-slate-700 bg-slate-950/95 p-3 pb-4">
         <div className="mb-2 text-[10px] font-black uppercase tracking-[.16em] text-emerald-300">Choose an investment</div>
         <div className="min-h-0 flex-1 overflow-y-auto pr-1"><div className="space-y-1.5">{visibleInterventions.map(item=>{const Icon=item.icon;const active=item.id===selectedId;const cost=costFor(item.actionType);return <button key={item.id} onClick={()=>setSelectedId(item.id)} className={`w-full rounded-xl border-2 px-3 py-1.5 text-left transition ${active?'border-amber-300 bg-amber-950/35':'border-slate-700 bg-slate-900 hover:border-emerald-500'}`}><div className="flex items-center gap-2"><Icon className={`h-4 w-4 shrink-0 ${active?'text-amber-300':'text-emerald-300'}`}/><b className="min-w-0 flex-1 text-[13px] leading-tight text-white">{item.title}</b><span className="text-[11px] font-black text-amber-300">{formatCurrency(cost)}</span></div></button>})}</div></div>
         <div className="mt-2 flex shrink-0 justify-center"><DisruptionMiniCard company={company} embedded/></div>
         <div aria-hidden="true" className="absolute -bottom-4 left-1/2 z-0 h-8 w-8 -translate-x-1/2 rotate-45 border-r border-b border-slate-700 bg-slate-950/95"/>
       </aside>
     </div>
     <section className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-slate-600 bg-slate-900/95 p-3">
