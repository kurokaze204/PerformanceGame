...
export const ActionsPanelV5:React.FC<Props>=({session,company,onPerformAction,onNextPhase})=>{
 ...
 <div className="fixed left-3 right-3 min-[1280px]:right-[360px] top-[94px] bottom-3 z-[90] rounded-3xl border border-slate-700 bg-[#080b12]/[0.985] shadow-2xl p-4 overflow-hidden flex flex-col" role="region" aria-label="Investment actions">
   ...
   <div className="mt-3 grid min-h-0 flex-1 grid-rows-[minmax(280px,1fr)_auto] gap-3">
     <div className="grid min-h-0 gap-3 min-[1050px]:grid-cols-[minmax(0,1fr)_300px]">
       <InvestmentRiverView company={company} mode={session.experienceMode} selectedDomain={domain} selectedSiteId={riverTargetSiteId} sourceSiteId={riverSourceSiteId} selectedExpertId={riverExpertId} highlightHQ={riverHighlightHQ} highlightAllSites={riverHighlightAllSites} highlightDomain/>
       <aside className="min-h-0 overflow-y-auto rounded-2xl border border-slate-700 bg-slate-950/80 p-3">
         <div className="mb-2 text-[10px] font-black uppercase tracking-[.16em] text-emerald-300">Choose an investment</div>
         <div className="space-y-2">{visibleInterventions.map(item=>{const Icon=item.icon;const active=item.id===selectedId;const cost=costFor(item.actionType);return <button key={item.id} onClick={()=>setSelectedId(item.id)} className={`w-full rounded-xl border-2 px-3 py-2.5 text-left transition ${active?'border-amber-300 bg-amber-950/35':'border-slate-700 bg-slate-900 hover:border-emerald-500'}`}><div className="flex items-center gap-2"><Icon className={`h-4 w-4 shrink-0 ${active?'text-amber-300':'text-emerald-300'}`}/><b className="min-w-0 flex-1 text-sm leading-tight text-white">{item.title}</b><span className="text-xs font-black text-amber-300">{formatCurrency(cost)}</span></div></button>})}</div>
       </aside>
     </div>
     <section className="rounded-2xl border border-slate-600 bg-slate-900/95 p-3">
       <div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-[10px] font-black uppercase tracking-[.16em] text-indigo-300">Selected investment</div><h3 className="text-lg font-black text-white">{selected.title}</h3></div><div className="text-sm font-black text-amber-300">{formatCurrency(totalCost)} · 1 Action</div></div>
       <div className="mt-3 grid gap-3 min-[1150px]:grid-cols-[minmax(0,1.15fr)_minmax(280px,.85fr)_220px]">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
         ...
         {needsSourceSite&&<label className="block text-[10px] uppercase text-slate-500 font-black">Teaching site<select value={sourceSiteId} onChange={e=>setSourceSiteId(e.target.value)} className="mt-1 w-full rounded-lg bg-slate-950 border border-slate-700 px-2.5 py-2 text-xs text-white normal-case">{activeSites.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>}
