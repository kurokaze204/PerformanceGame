...
const LESSONS:Record<number,{eyebrow:string;heading:string;intro:string;items:Lesson[]}>= {
  1:{
    eyebrow:'Newbie mode · Round 1', heading:'Start with what the company already knows',
    intro:'Before changing anything, look at the capability already inside the company. Different sites know different things, corporate knowledge can be available more broadly, and a few people may hold unusually deep expertise.',
    items:[
      {title:'Organisational knowledge',icon:BookOpen,p1:'Teams carry practical know-how. The Corporate Intranet can make organisational knowledge available beyond the site where it originated. Having access, however, is not the same as having enough local capability to understand and apply what you find.',p2:'For the opening Events, inspect what each site already knows and use the Corporate Intranet when it helps. The first challenge will show why knowledge can still fail to reach the point of need.',interventions:'Local Team · Corporate Intranet'},
      {title:'Deep Experts',icon:Users,p1:'Experts provide judgement, pattern recognition and depth that is difficult to capture completely. They are powerful precisely because they know more than the surrounding organisation, but relying on one person for critical capability can create a Single Point of Failure.',p2:'You will be able to build and spread expert capability later. For now, notice where expertise sits and where the company is vulnerable if that person is unavailable.',interventions:'Expert capability · Single Point of Failure visibility'}
    ]
  },
  2:{
...
export const NewbieLearningOverlay:React.FC<Props>=({session,onContinue})=>{
  ...
  return <div className="w-full max-w-4xl mx-auto mt-4 rounded-3xl border-2 border-indigo-500 bg-slate-950/97 p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="newbie-lesson-title">
    <div className="text-xs uppercase tracking-[.2em] text-indigo-300 font-black">{lesson.eyebrow}</div>
    ...
    <div className={`grid gap-3 mt-4 ${lesson.items.length>1?'md:grid-cols-2':'grid-cols-1'}`}>{lesson.items.map(item=>{const Icon=item.icon;return <section key={item.title} className="rounded-2xl border border-slate-700 bg-slate-900/75 p-4"><div className="flex items-center gap-2"><Icon className="w-5 h-5 text-indigo-300" aria-hidden="true"/><h3 className="font-black text-white">{item.title}</h3></div><p className="text-sm leading-relaxed text-slate-300 mt-2">{item.p1}</p><p className="text-sm leading-relaxed text-slate-400 mt-2">{item.p2}</p><div className="mt-3 rounded-lg border border-emerald-800 bg-emerald-950/30 px-3 py-2 text-xs text-emerald-200"><b>Visible now:</b> {item.interventions}</div></section>})}</div>
