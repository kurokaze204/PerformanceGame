...
const TrendChart: React.FC<{ title: string; rounds: number[]; series: Series[]; enabled: Record<string, boolean>; scales?: Record<string, Scale> }> = ({ title, rounds, series, enabled, scales }) => {
  ...
  const pointsFor = (s: Series) => {
    ...
    const min = supplied?.min ?? Math.min(0, ...s.values);
    const max = supplied?.max ?? Math.max(0, ...s.values);
    const span = Math.max(1, max - min);
    return s.values.map((v, i) => {
      const x = left + (s.values.length <= 1 ? innerW / 2 : i * innerW / (s.values.length - 1));
...
export const CompanyChartsOverlay: React.FC<Props> = ({ company, snapshots, onClose, showIntro = false }) => {
  ...
  const siteSeries=activeSites.map(site=>commonSeries(site.id));
  const siteScales=useMemo<Record<string,Scale>>(()=>{
    const scales:Record<string,Scale>={};
    const ids=siteSeries[0]?.map(s=>s.id)||[];
    for(const id of ids){
      const values=siteSeries.flatMap(list=>list.find(s=>s.id===id)?.values||[]);
      scales[id]={min:Math.min(0,...values),max:Math.max(0,...values)};
    }
    return scales;
  },[siteSeries]);
    ...
    <div className="h-full max-w-[1600px] mx-auto rounded-3xl border border-indigo-700 bg-slate-900 shadow-2xl flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-[minmax(0,1fr)_230px] gap-3 p-3">
        <div className="overflow-auto grid grid-cols-2 xl:grid-cols-3 gap-2 content-start">
          ...
          {activeSites.map((site,index)=><TrendChart key={site.id} title={site.name} rounds={rounds} series={siteSeries[index]} enabled={enabled} scales={siteScales}/>)}
        </div>
      ...
      {intro && <div className="absolute inset-0 z-10 grid place-items-center bg-slate-950/75 p-6"><div className="max-w-xl rounded-3xl border-2 border-indigo-500 bg-slate-950 p-6 shadow-2xl"><div className="text-xs uppercase tracking-[0.18em] text-indigo-300 font-black">Round 3 — Charts unlocked</div><h3 className="text-2xl font-black text-white mt-2">Watch the shape of your company change.</h3><p className="text-sm text-slate-300 mt-3 leading-relaxed">These compact charts show turnover, local knowledge, codified knowledge, domain strength, corporate knowledge and knowledge investment across rounds. Each factor is scaled consistently across all city charts, so relative height can be compared directly between sites. Use the single legend on the right to turn a measure on or off across every chart at once.</p><p className="text-sm text-slate-300 mt-3">Close this view with the X. From now on you can reopen it at any time using the <b>Charts</b> button at the top of the game.</p><button onClick={()=>setIntro(false)} className="mt-5 w-full rounded-xl bg-indigo-500 py-3 font-black text-white">VIEW MY COMPANY</button></div></div>}
    </div>
  </div>;
};
