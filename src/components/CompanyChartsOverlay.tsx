...
const TrendChart: React.FC<{ title: string; rounds: number[]; series: Series[] }> = ({ title, rounds, series }) => {
  const [enabled, setEnabled] = useState<Record<string, boolean>>(() => Object.fromEntries(series.map((s, i) => [s.id, i < 3])));
  const shown = series.filter((s) => enabled[s.id]);
  const width = 760, height = 180, left = 28, right = 18, top = 14, bottom = 28;
  ...
  return <section className="rounded-2xl border border-slate-700 bg-slate-950/85 p-3 min-w-0">
    <div className="font-black text-white text-sm mb-2">{title}</div>
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[170px] bg-slate-900/55 rounded-xl">
      {rounds.map((r,i)=>{const x=left+(rounds.length<=1?innerW/2:i*innerW/(rounds.length-1));return <g key={r}><line x1={x} y1={top} x2={x} y2={top+innerH} stroke="#334155" strokeWidth="1"/><text x={x} y={height-8} textAnchor="middle" fill="#94a3b8" fontSize="11">R{r}</text></g>})}
      {shown.map((s,i)=><polyline key={s.id} points={pointsFor(s.values)} fill="none" stroke={LINE_COLORS[series.findIndex(x=>x.id===s.id)%LINE_COLORS.length]} strokeWidth="3" strokeLinejoin="round" strokeLinecap="round"/>)}
    </svg>
    <div className="flex flex-wrap gap-1.5 mt-2">
      {series.map((s,i)=><button key={s.id} onClick={()=>setEnabled(v=>({...v,[s.id]:!v[s.id]}))} className={`rounded-full border px-2 py-1 text-[10px] font-bold ${enabled[s.id]?'border-slate-400 bg-slate-800 text-white':'border-slate-800 bg-slate-950 text-slate-600'}`}><span className="inline-block w-2 h-2 rounded-full mr-1" style={{background:LINE_COLORS[i%LINE_COLORS.length]}}/>{s.label}</button>)}
    </div>
  </section>;
};
  ...
  return <div className="fixed inset-0 z-[160] bg-[#080b12]/98 backdrop-blur-sm p-3 md:p-5 overflow-hidden">
    <div className="h-full max-w-[1600px] mx-auto rounded-3xl border border-indigo-700 bg-slate-900 shadow-2xl flex flex-col overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-700 flex items-start justify-between gap-4 shrink-0">
        <div><div className="text-xs uppercase tracking-[0.18em] text-indigo-300 font-black">Company Trends</div><h2 className="text-xl font-black text-white">How your capability is changing round by round</h2><p className="text-xs text-slate-400 mt-1">Trend view only: there is deliberately no Y-axis scale. Toggle any measure in each chart legend. Each line is independently scaled to emphasise direction rather than absolute magnitude.</p></div>
      ...
      <div className="flex-1 overflow-auto p-4 grid xl:grid-cols-2 gap-3">
        <TrendChart title="Corporate HQ" rounds={rounds} series={commonSeries()} />
        {company.sites.filter(s=>!s.isClosed).map(site=><TrendChart key={site.id} title={site.name} rounds={rounds} series={commonSeries(site.id)} />)}
      </div>
      {intro && <div className="absolute inset-0 z-10 grid place-items-center bg-slate-950/75 p-6"><div className="max-w-xl rounded-3xl border-2 border-indigo-500 bg-slate-950 p-6 shadow-2xl"><div className="text-xs uppercase tracking-[0.18em] text-indigo-300 font-black">Round 2 — Charts unlocked</div><h3 className="text-2xl font-black text-white mt-2">Watch the shape of your company change.</h3><p className="text-sm text-slate-300 mt-3 leading-relaxed">These charts let you see how turnover, local knowledge, codified knowledge, domain strength, corporate knowledge and knowledge investment are changing across rounds. Use the legend beneath each chart to turn individual lines on and off. There is no Y-axis scale because this view is about trends, not comparing unlike units.</p><p className="text-sm text-slate-300 mt-3">Close this view with the X. From now on you can reopen it at any time using the <b>Charts</b> button at the top of the game.</p><button onClick={()=>setIntro(false)} className="mt-5 w-full rounded-xl bg-indigo-500 py-3 font-black text-white">VIEW MY COMPANY</button></div></div>}
    </div>
  </div>;
};
