...
export const RiverDiagramOverlay:React.FC<Props>=({company,mode,phase,onClose,onShare})=>{
 ...
 const W=920,H=350,padL=58,padR=28,padT=24,padB=52;
 ...
 return <div className="fixed inset-0 z-[220] bg-black/75 backdrop-blur-sm p-3 sm:p-4 flex items-center justify-center overflow-hidden" role="dialog" aria-modal="true" aria-label="Knowledge Transfer">
   <div className="w-full max-w-6xl h-[min(90vh,820px)] rounded-3xl border border-slate-600 bg-[#0b1020] shadow-2xl overflow-hidden flex flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5">
     <div className="mt-4 grid lg:grid-cols-[minmax(0,1fr)_300px] gap-4 items-stretch">
      <div className="min-h-0 flex flex-col gap-2">
       <div className="rounded-2xl border border-slate-700 bg-slate-950 p-3 overflow-hidden flex-1 min-h-0">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[min(33vh,330px)] min-h-[250px]" role="img" aria-label="Knowledge Transfer map showing site and expert knowledge scores by domain">
         ...
         {data.map((d,i)=><g key={d.domain}>
           <text x={x(i)} y={H-18} textAnchor="middle" fill="#e2e8f0" fontSize="13" fontWeight="700">{DOMAIN_INFO[d.domain].label}</text>
       ...
       <div className="grid md:grid-cols-3 gap-2 text-[11px] leading-snug">
        <div className="rounded-xl border border-emerald-800 bg-emerald-950/30 p-2"><b className="text-emerald-300">Strongest site</b><div className="text-slate-300 mt-0.5">Best site-level knowledge. Yellow people show expert knowledge that may sit above it.</div></div>
        <div className="rounded-xl border border-sky-800 bg-sky-950/30 p-2"><b className="text-sky-300">Transfer gap</b><div className="text-slate-300 mt-0.5">Knowledge the company already has but has not yet moved to every place that needs it.</div></div>
        <div className="rounded-xl border border-emerald-800 bg-emerald-950/30 p-2"><b className="text-emerald-300">Weakest site</b><div className="text-slate-300 mt-0.5">The lowest current site score — often the best place to target transfer first.</div></div>
       </div>
      </div>
      <aside className="rounded-2xl border border-emerald-800 bg-emerald-950/20 p-4"><div className="text-xs uppercase tracking-wider text-emerald-300 font-black">Knowledge Transfer</div><h3 className="text-lg font-black text-white mt-1">Move knowledge to where it is needed</h3><p className="text-sm text-slate-300 mt-1">Use a stronger site as the teacher. The receiving site can reach about <b>80%</b> of what that teaching site currently knows in the selected domain. This is more targeted than publishing the knowledge to the Corporate Intranet.</p>
       ...
       <label className="block mt-2 text-xs uppercase text-slate-400 font-black">Receiving site<select value={targetSiteId} onChange={e=>{setTargetSiteId(e.target.value);setMessage('')}} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white normal-case">{[...selectedData.scores].sort((a,b)=>a.score-b.score).map(({site,score})=><option key={site.id} value={site.id}>{site.name} · {score}</option>)}</select></label>
       <div className="mt-2 grid grid-cols-3 gap-1 text-center"><Metric label="Teacher" value={sourceScore}/><Metric label="Team now" value={currentTarget}/><Metric label="After" value={transferTarget}/></div>
       {phase==='investment'?<button onClick={share} disabled={!canBenefit||busy} className="mt-2 w-full rounded-xl bg-amber-400 py-2.5 font-black text-slate-950 disabled:bg-slate-800 disabled:text-slate-500">{busy?'TRANSFERRING…':`TRANSFER · 1 ACTION · ${formatCurrency(TRANSFER_COST)}`}</button>:<div className="mt-2 rounded-xl border border-slate-700 bg-slate-950 p-2 text-center text-xs text-slate-400">Planning view available now. Execute Knowledge Transfer during the <b className="text-white">Invest</b> phase.</div>}
