    ...
    <div className="mt-4 grid lg:grid-cols-[minmax(0,1fr)_300px] gap-4">
     <div className="rounded-2xl border border-slate-700 bg-slate-950 p-3 overflow-hidden">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[440px]" role="img" aria-label="River diagram showing site knowledge scores by domain">
       ...
       <path d={riverFill} fill="#0c4a6e" fillOpacity="0.78"/>
     ...
     <aside className="rounded-2xl border border-sky-800 bg-sky-950/20 p-4"><div className="text-[10px] uppercase tracking-wider text-sky-300 font-black">Knowledge Sharing</div><h3 className="text-lg font-black text-white mt-1">Shrink the river</h3><p className="text-xs text-slate-300 mt-1">Use a stronger site as the teacher. The receiving site can reach about <b>80%</b> of what that teaching site currently knows in the selected domain. This is deliberately stronger and more targeted than publishing generic knowledge to the Corporate Intranet.</p>
      <label className="block mt-3 text-[10px] uppercase text-slate-400 font-black">Domain<select value={domain} onChange={e=>{setDomain(e.target.value as KnowledgeDomain);setMessage('')}} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white normal-case">{DOMAINS.map(d=><option key={d} value={d}>{DOMAIN_INFO[d].label}</option>)}</select></label>
      <label className="block mt-2 text-[10px] uppercase text-slate-400 font-black">Teaching site<select value={sourceSiteId} onChange={e=>{setSourceSiteId(e.target.value);setMessage('')}} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white normal-case">{[...selectedData.scores].sort((a,b)=>b.score-a.score).map(({site,score})=><option key={site.id} value={site.id}>{site.name} · {score}</option>)}</select></label>
      <div className="my-2 flex justify-center text-sky-300"><ArrowRight className="w-5 h-5"/></div>
      <label className="block text-[10px] uppercase text-slate-400 font-black">Receiving site<select value={targetSiteId} onChange={e=>{setTargetSiteId(e.target.value);setMessage('')}} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 p-2 text-sm text-white normal-case">{[...selectedData.scores].sort((a,b)=>a.score-b.score).map(({site,score})=><option key={site.id} value={site.id}>{site.name} · {score}</option>)}</select></label>
      ...
      {phase==='investment'?<button onClick={share} disabled={!canBenefit||busy} className="mt-3 w-full rounded-xl bg-amber-400 py-2.5 font-black text-slate-950 disabled:bg-slate-800 disabled:text-slate-500">{busy?'SHARING…':`SHARE · 1 ACTION · ${formatCurrency(18)}`}</button>:<div className="mt-3 rounded-xl border border-slate-700 bg-slate-950 p-2 text-center text-xs text-slate-400">Planning view available now. Execute Knowledge Sharing during the <b className="text-white">Invest</b> phase.</div>}
      {!canBenefit&&source&&target&&<div className="mt-2 text-[10px] text-amber-300">Choose a stronger teaching site or a weaker receiving site; this pairing would not lift Team Capability.</div>}{message&&<div className="mt-2 rounded-lg border border-emerald-800 bg-emerald-950/30 p-2 text-xs text-emerald-200">{message}</div>}
     </aside>
    </div>
    <div className="mt-3 grid md:grid-cols-3 gap-3 text-xs"><div className="rounded-xl border border-emerald-800 bg-emerald-950/30 p-3"><b className="text-emerald-300">North bank</b><div className="text-slate-300 mt-1">The most your organisation currently knows in each domain. Sites here are natural teachers. Above this bank is where genuinely new outside expertise may be needed.</div></div><div className="rounded-xl border border-sky-800 bg-sky-950/30 p-3"><b className="text-sky-300">The river</b><div className="text-slate-300 mt-1">The internal performance gap. Your KM strategy should deliberately narrow it by moving proven knowledge from stronger sites to weaker sites.</div></div><div className="rounded-xl border border-emerald-800 bg-emerald-950/30 p-3"><b className="text-emerald-300">South bank</b><div className="text-slate-300 mt-1">The lowest current site score in each domain — often the best place to target internal knowledge transfer first.</div></div></div>
   </div>
 </div>;
};

const Metric:React.FC<{label:string;value:number|string}>=({label,value})=><div className="rounded-lg border border-sky-900 bg-slate-950 p-2"><div className="text-[8px] uppercase text-slate-500 font-black">{label}</div><div className="text-xl font-black text-white">{value}</div></div>;
