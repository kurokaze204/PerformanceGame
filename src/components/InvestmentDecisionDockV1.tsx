...
export const InvestmentDecisionDockV1:React.FC<Props>=({session,company,selectedSiteId,tool,onTool,onSelectSite,onSelectHQ,onSelectExpert})=>{
  ...
  return <div className="fixed right-4 top-[104px] z-[145] flex flex-col items-end gap-2" aria-label="Investment decision support">
    <div className="rounded-2xl border-2 border-violet-700 bg-[#0b0f18]/95 p-2 shadow-2xl backdrop-blur-md">
      <div className="px-2 pb-1 text-[11px] font-black uppercase tracking-[.16em] text-emerald-300">Decision support</div>
      <nav className="flex gap-2" aria-label="Investment tools">
        {tabs.map(([id,label,Icon])=><button key={id} onClick={()=>onTool(tool===id?null:id)} className={`min-h-12 rounded-xl border-2 px-3 py-2 flex items-center gap-2 text-sm font-black transition ${tool===id?'border-violet-300 bg-violet-950 text-white':'border-slate-700 bg-slate-950 text-slate-200 hover:border-emerald-400'}`}><Icon className="h-5 w-5"/><span className="hidden sm:inline">{label}</span></button>)}
      </nav>
    </div>
    {tool&&<aside className="max-h-[calc(100vh-180px)] w-[min(500px,calc(100vw-24px))] overflow-y-auto rounded-2xl border-2 border-violet-500 bg-[#0b0f18]/[0.985] p-5 shadow-2xl">
