...
export const RiskRiverDiagram:React.FC<Props>=({company,mode,effect})=>{
 ...
 const W=900,H=260,padL=46,padR=22,padT=18,padB=42;
 ...
 const expertFigure=(key:string,px:number,py:number,kind:'normal'|'departed'|'replacement')=>{
  if(kind==='normal')return <g key={key}><circle cx={px} cy={py-6} r="4.3" fill="#facc15"/><path d={`M ${px} ${py-1} L ${px} ${py+9} M ${px-6} ${py+3} L ${px+6} ${py+3} M ${px} ${py+9} L ${px-5} ${py+16} M ${px} ${py+9} L ${px+5} ${py+16}`} stroke="#facc15" strokeWidth="3" strokeLinecap="round"/></g>;
  if(kind==='departed')return <motion.g key={key} initial={{opacity:1}} animate={{opacity:[1,1,0]}} transition={{duration:1.35,times:[0,.68,1]}}><motion.circle cx={px} cy={py-6} r="4.3" fill="#facc15" animate={{r:[4.3,7,4.3]}} transition={{duration:.45}}/><path d={`M ${px} ${py-1} L ${px} ${py+9} M ${px-6} ${py+3} L ${px+6} ${py+3} M ${px} ${py+9} L ${px-5} ${py+16} M ${px} ${py+9} L ${px+5} ${py+16}`} stroke="#facc15" strokeWidth="3" strokeLinecap="round"/><motion.g initial={{opacity:0}} animate={{opacity:[0,1,1,0]}} transition={{duration:1.35,times:[0,.25,.78,1]}}><path d={`M ${px-10} ${py-14} L ${px+10} ${py+17} M ${px+10} ${py-14} L ${px-10} ${py+17}`} stroke="#ef4444" strokeWidth="4.5" strokeLinecap="round"/></motion.g></motion.g>;
  const rise=y(0)-py;
  return <motion.g key={key} initial={{y:rise,opacity:.25}} animate={{y:0,opacity:1}} transition={{duration:.8,ease:'easeOut'}}><motion.circle cx={px} cy={py-6} r="4.3" initial={{fill:'#22c55e'}} animate={{fill:['#22c55e','#22c55e','#facc15']}} transition={{duration:1.15,times:[0,.7,1]}}/><motion.path d={`M ${px} ${py-1} L ${px} ${py+9} M ${px-6} ${py+3} L ${px+6} ${py+3} M ${px} ${py+9} L ${px-5} ${py+16} M ${px} ${py+9} L ${px+5} ${py+16}`} initial={{stroke:'#22c55e'}} animate={{stroke:['#22c55e','#22c55e','#facc15']}} transition={{duration:1.15,times:[0,.7,1]}} strokeWidth="3" strokeLinecap="round"/></motion.g>;
 };
 return <div className="rounded-2xl border border-slate-700 bg-slate-950/90 p-3">
  <div className="flex items-center justify-between gap-3 px-1"><div className="text-[10px] uppercase tracking-[.16em] font-black text-emerald-300">Knowledge River</div><div className="text-[10px] text-slate-500">white · sites &nbsp; yellow · experts</div></div>
