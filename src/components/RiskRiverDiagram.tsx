...
export const RiskRiverDiagram:React.FC<Props>=({company,mode,effect,focus=null})=>{
 ...
 const W=900,H=330,padL=58,padR=50,padT=54,padB=52;
 ...
 const expertFigure=(key:string,px:number,py:number,kind:'normal'|'departed'|'replacement',ring?:'ok'|'spof',assessed=false)=>{
  const star=assessed?<g><polygon points={starPoints(px,py,38,23,7)} fill="#facc15" fillOpacity=".09" stroke="#fde047" strokeWidth="4.5" strokeLinejoin="round" filter="url(#risk-star-glow)"/><text x={px} y={py-45} textAnchor="middle" fill="#fde047" fontSize="9" fontWeight="900" paintOrder="stroke" stroke="#020617" strokeWidth="3">UNDER TEST</text></g>:null;
 ...
 const focusSitePath=focusSite?data.map((d,di)=>{const si=d.scores.findIndex(entry=>entry.site.id===focusSite.id);if(si<0)return'';const jitter=(si-(d.scores.length-1)/2)*6;return `${di?'L':'M'} ${x(di)+jitter} ${y(d.scores[si].score)}`}).filter(Boolean).join(' '):'';
