...
interface Props{roll:number;threshold:number;sides:number;kind:Kind;isSPOF?:boolean;onComplete?:()=>void;onNext?:()=>void;nextLabel?:string;nextDisabled?:boolean;}

const sleep=(ms:number)=>new Promise<void>(resolve=>window.setTimeout(resolve,ms));

export const RiskRollTrack:React.FC<Props>=({roll,threshold,sides,kind,isSPOF,onComplete,onNext,nextLabel='NEXT',nextDisabled=false})=>{
 ...
 const buttonLabel=rolling?'Rolling…':done?nextLabel:'Push to Roll';
 const click=()=>{if(done){onNext?.();return;}void pushToRoll();};

 return <div className="w-[350px] max-w-full rounded-2xl border-2 border-sky-700 bg-slate-950 px-3 py-2.5 shadow-lg">
