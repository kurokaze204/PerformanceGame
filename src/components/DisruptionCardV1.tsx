...
export const DisruptionDealOverlay:React.FC<{session:GameSessionV2;company:CompanyV2;onContinue:()=>void}>=({session,company,onContinue})=>{
 ...
 return <div className="w-full max-w-2xl mx-auto mt-8 rounded-3xl border-2 border-amber-500 bg-[#0d0f15]/95 p-6 shadow-2xl">
   ...
   <div className="mt-6 flex items-center justify-center min-h-[300px]">
    {!revealed?<button onClick={()=>setRevealed(true)} className="relative h-[260px] w-[186px] rounded-[18px] border-[5px] border-amber-300 bg-[radial-gradient(circle_at_50%_24%,#6b4517_0%,#2a1808_55%,#0a0806_100%)] shadow-[0_20px_45px_rgba(0,0,0,.55)]"><div className="absolute inset-4 rounded-xl border-2 border-amber-200/30"/><div className="absolute -left-3 -top-3 h-full w-full rounded-[18px] border border-amber-700 bg-[#120d08] -z-10"/><b className="text-xl tracking-[.16em] text-amber-100">DISRUPTION</b><span className="absolute left-0 right-0 bottom-5 text-[10px] font-black uppercase tracking-widest text-amber-300">Click to deal</span></button>
