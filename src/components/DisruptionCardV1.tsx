...
export const DisruptionMiniCard:React.FC<{company:CompanyV2;aboveOverlay?:boolean;embedded?:boolean;wide?:boolean}>=({company,aboveOverlay=false,embedded=false,wide=false})=>{
 const card=company.disruptionCard;
 if(!card)return null;
 const shell=wide
  ?'relative w-[240px] min-h-[116px] rounded-[14px] border-[3px] border-amber-300 bg-[#171109] p-2.5 shadow-[0_8px_18px_rgba(0,0,0,.45)] pointer-events-auto'
  :embedded
 ...
 return <div className={shell} aria-label="Known disruption card">
  ...
  <div className={wide?'mt-2 grid grid-cols-2 gap-1.5':`${embedded?'mt-1.5 space-y-1':'mt-2 space-y-1.5'}`}>{card.domains.map(req=><div key={req.domain} className={`rounded-md border border-amber-900/70 bg-black/25 ${wide?'min-h-[54px] px-2 py-1.5':embedded?'min-h-[39px] px-1.5 py-1':'min-h-[44px] p-1.5'}`}><div className="flex items-start gap-1"><span className="mt-0.5 h-2 w-2 shrink-0 rounded-full" style={{backgroundColor:DOMAIN_INFO[req.domain].color}}/><span className={`min-w-0 flex-1 break-normal [overflow-wrap:normal] font-black leading-[1.05] text-slate-200 ${wide?'text-[8px]':embedded?'text-[7px]':'text-[9px]'}`}>{DOMAIN_INFO[req.domain].label}</span></div><div className={`${wide?'mt-1 text-base':embedded?'mt-0 text-sm':'mt-0.5 text-lg'} text-right leading-none font-black text-amber-100`}>{req.difficulty}</div></div>)}</div>
 </div>;
};
