...
export const AARDebriefView: React.FC<AARDebriefViewProps> = ({ session, company, onClose, onOpenCharts }) => {
  ...
  if(finished)return <div className="fixed left-0 right-0 top-[var(--tpg-header-height)] bottom-0 z-[240] bg-[#080b12] text-slate-200 p-4 overflow-auto grid place-items-center">
    <div className="w-full max-w-3xl rounded-3xl border-2 border-emerald-700 bg-slate-900 p-7 md:p-10 shadow-2xl text-center">
      ...
      <button onClick={startNewGame} className="tpg-action tpg-action-muted mt-7">Start a new game</button>
    </div>
  </div>;

  return (
    <div className="fixed left-0 right-0 top-[var(--tpg-header-height)] bottom-0 z-[220] bg-[#080b12]/[0.995] text-slate-200 p-3 md:p-5 overflow-auto">
      <div className="max-w-5xl mx-auto rounded-3xl border border-indigo-700 bg-slate-900 shadow-2xl overflow-hidden">
        <header className="px-5 py-4 border-b border-slate-700">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex gap-2 shrink-0">{onOpenCharts&&<button onClick={onOpenCharts} className="tpg-action !h-10 !px-3 !text-xs"><BarChart3 className="w-4 h-4"/>Charts</button>}<button onClick={onClose} className="tpg-action tpg-action-muted !h-10 !px-3 !text-xs"><ArrowLeft className="w-4 h-4"/>Final result</button><button onClick={finishGame} className="tpg-action !h-10 !px-3 !text-xs border-emerald-400 bg-emerald-500 text-slate-950 hover:bg-emerald-400"><CheckCircle2 className="w-4 h-4"/>Finish game</button></div>
          </div>
        ...
        <main className="p-5">
          <section className="rounded-2xl border border-slate-700 bg-slate-950/55 p-5">
            ...
            <div className="mt-5 flex justify-between items-center border-t border-slate-800 pt-4 gap-4"><span className="text-xs text-slate-500">People can pass on any question. The conversation matters more than completing every box.</span>{qIndex < QUESTIONS.length - 1 ? <button onClick={goNext} className="tpg-action tpg-action-primary shrink-0">Next question <ArrowRight className="w-4 h-4"/></button> : <button onClick={finishGame} className="tpg-action shrink-0 border-emerald-400 bg-emerald-500 text-slate-950 hover:bg-emerald-400"><CheckCircle2 className="w-4 h-4"/>Finish game</button>}</div>
          </section>
