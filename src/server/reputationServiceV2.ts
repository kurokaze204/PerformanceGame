...
const FAVOUR_MESSAGES = [
  'Your old uni friend really came through for you this time.',
...
function applySiteDelta(company: any, site: any, delta: number): void { company.turnover = company.turnover; site.turnover = Math.max(0, Math.round(site.turnover + delta)); recalcCompanyTurnover(company); }
function applyCompanyDelta(company: any, delta: number): void {
  const active = company.sites.filter((s: any) => !s.isClosed); if (!active.length || delta === 0) return;
  const total = active.reduce((sum: number, s: any) => sum + s.turnover, 0); let remaining = Math.round(delta);
  active.forEach((site: any, idx: number) => { const share = idx === active.length - 1 ? remaining : Math.round(delta * (total > 0 ? site.turnover / total : 1 / active.length)); site.turnover = Math.max(0, site.turnover + share); remaining -= share; }); recalcCompanyTurnover(company);
}
...
export async function resolveWithReputationV2(sessionId: string, companyId: string, eventInstanceId: string) {
  ...
  event.turnoverChangeApplied = turnoverChange; event.netFinancialImpact = turnoverChange;
  event.domainResults = event.card.domains.map((r) => ({ domain:r.domain,baseKnowledge:0,usableIntranet:0,team:0,localCodified:0,expertBonus:0,copBonus:0,automationBonus:0,consultantBonus:0,totalKnowledge:0,difficulty:r.difficulty,dieRoll:0,requiredTotal:r.difficulty,achievedTotal:r.difficulty,domainSuccess:true,reputationOverride:true,explanation:flavourText }));
