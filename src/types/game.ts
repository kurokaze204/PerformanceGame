...
export const DOMAIN_INFO: Record<KnowledgeDomain, { label: string; color: string; bgClass: string; textClass: string; borderClass: string; icon: string }> = {
  ...
  hr: {
    ...
    color: '#dc2626', // Red
    bgClass: 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300',
...
export interface Expert {
  id: string;
  name: string;
  domains: { domain: KnowledgeDomain; score: number }[];
  location: string; // Site ID or 'HQ'
  homeLocation: string;
  state: ExpertState;
  isSPOF: boolean;
  spofDomains: KnowledgeDomain[];
  isVacant?: boolean;
  avatarSeed?: string;
}
...
export interface Site {
  ...
  isRDSite: boolean;
  isClosed: boolean;
  teamCapability: DomainScoreMap;
  codifiedKnowledge: DomainScoreMap;
  coordinates: { x: number; y: number }; // Relative coordinates on map (0-100)
}
...
export interface Company {
  ...
  name: string;
  turnover: number;
  startingTurnover: number;
  intranet: DomainScoreMap;
  intranetRoundGrowth: DomainScoreMap;
  automatedDomains: KnowledgeDomain[];
  horizonScanDomain: KnowledgeDomain | null;
  horizonScanUsedThisRound: boolean;
  actionsRemaining: number;
  sites: Site[];
  experts: Expert[];
  auditedSiteId: string | null;
}

export type EventType = 'problem' | 'opportunity';
export type EventScope = 'local' | 'enterprise';

export interface EventDomainRequirement {
...
export interface EventCard {
  ...
  title: string;
  description: string;
  domains: EventDomainRequirement[];
  impact: number;
  rdSiteOnly?: boolean;
  tags: string[];
}

export interface ActiveEventAllocation {
  expertId?: string;
  useCoPSupport?: boolean;
  copContributingCompanyId?: string;
  copContributingExpertId?: string;
}

export interface ActiveEvent {
  instanceId: string;
  card: EventCard;
  targetSiteId?: string; // For local events
  allocations: Record<KnowledgeDomain, ActiveEventAllocation>; // Domain -> allocation
  isResolved: boolean;
  success?: boolean;
  domainResults?: {
    domain: KnowledgeDomain;
  ...
  turnoverChangeApplied?: number;
  experientialLearningAwarded?: boolean;
}
...
export interface CoPMembership {
  companyId: string;
  domain: KnowledgeDomain;
  expertId: string;
  activeRound: number;
...
export interface Participant {
  ...
  name: string;
  companyId: string;
  role: 'participant' | 'controller' | 'facilitator';
  lastSeen: string;
}

export interface GameSession {
  id: string; // Session Code (e.g. KM2026)
  title: string;
  name?: string;
  round: number; // 1 to 5, 6 is Final Disruption
  phase: GamePhase;
  isPaused: boolean;
  isFinalDisruptionActive: boolean;
  finalDisruptionCard?: EventCard;
  finalDisruptionResolved?: boolean;
  companies: Company[];
  activeEvents: Record<string, ActiveEvent[]>; // CompanyId -> ActiveEvent[]
  copMemberships: CoPMembership[];
