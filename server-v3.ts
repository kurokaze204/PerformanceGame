...
import type { BusinessStrategy, ExperienceMode, KnowledgeStrategy, PopulationMode } from './src/types/gameV2.ts';
...
async function startServer() {
  ...
  app.post('/api/sessions', async (req, res) => {
    try {
      const { sessionId, title, name, companyNames, companyCount, experienceMode, gameDurationMinutes, maxPlayersPerCompany, actionsPerRound, populationMode } = req.body || {};
  ...
  app.post('/api/ai/debrief', async (req, res) => {
    ...
    res.json({ summary: 'Use the evidence to ask what was planned, what happened, why it differed, and what the team would do better.', facilitatorQuestions: ['What did you plan to happen?', 'What actually happened?', 'Why was it different?', 'What would you improve next time in real life?'], recordedEvents: logs.length });
  });
